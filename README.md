...
| [docs/DEPLOY-LXC.md](docs/DEPLOY-LXC.md) | *Optional*: the two settings an unprivileged Proxmox LXC needs. Ignore it on any other host |
...
|---|---|---|---|
| mapa-emergencia | none | **closed by the source** (was ~930) | Feed agreed with the owner, who published `/api/publico` on request; it carried staffing per point, which no other source had. **The map closed on 2026-08-31.** All three Worker routes answer `410 Gone` with `{"cerrado":true,"archivo":…}`, the site serves a signed closure notice, and every record link now lands on it. That is an explicit withdrawal, not an absence, so its records were retired with `pnpm ingest mapa-emergencia --retirar` — kept in the database, out of search. Their [archive](https://mapa-emergencia-archivo.netlify.app) is a retrospective, not a feed. |
...
| corag (`ayuda.corag.org`) | allows everything | **ready, disabled** (15) | Read through the **Cabuya Protocol**, not a bespoke adapter: they publish a manifest at `/.well-known/cabuya.json` naming a place feed, which is what the appendix of [docs/PROPUESTA-CORAG.md](docs/PROPUESTA-CORAG.md) asked them for. Collection centres in Pereira, Dosquebradas and Bogotá. Their feed carries no contact values at all — §7.2 sends `contact_available`, the fact without the number — so nothing is mirrored and `mirrorsContacts` stays off. Their MCP endpoint and write API are never read. What is missing on their side is what a person actually needs: names are placeholders, there is no address, and the fifteen share one `last_confirmed_at`. |
