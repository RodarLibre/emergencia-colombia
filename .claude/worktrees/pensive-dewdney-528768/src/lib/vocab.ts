...
export const CATEGORY_LABELS: Record<Category, string> = {
  ...
  medical_assistance: "Atencion medica",
  shelter: "Alojamiento",
  ...
  cash_or_donation: "Donacion en dinero",
 ...
 * exactly the error the plan prohibits in section 7.2.
 */
export function verificationLabel(level: VerificationLevel, sourceName: string): string {
  switch (level) {
    case "official":
      return `Fuente oficial (${sourceName})`;
    case "source_verified":
      return `Verificado por ${sourceName}`;
    case "community_unverified":
      return "Reporte comunitario sin verificar";
    case "disputed":
      return "Fuentes en desacuerdo";
    case "unknown":
      return "Sin informacion de verificacion";
  }
}
...
export const FRESHNESS_LABELS: Record<FreshnessState, string> = {
  ...
  needs_reconfirmation: "No reconfirmado recientemente",
  stale: "Sin confirmar hace mucho",
};
